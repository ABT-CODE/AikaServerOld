...
type
  ...
  TMobsStructDungeonInstance = packed record
    ...
    MobExp: DWORD;
    MobType: WORD;
    MobElevation, Cabeca, Perna: WORD; // mob "altura"
...
begin
  try
    ...
    InstanceID := Servers[Self.Base.ChannelId].Players[Self.AttackerID].DungeonInstanceID;
    ...
    begin
      ...
      Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.LastReceivedAttack := Now;
      Packet.MobCurrHP := Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Character.CurrentScore.CurHP;
      Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.SendToVisible(Packet, Packet.Header.size, True);
      Exit;
    end;
    ...
    case Self.MobLevel of
      0 .. 200: Dano := 1000;
      201 .. 500: Dano := 2000;
      501 .. 1000: Dano := 3000;
      1001 ..1500: Dano := 4000;
      1501 .. 2000: Dano := 10000;
      2001 .. 2500: Dano := 15000;
      2501 .. 3000: Dano := 25000;
      3001 .. 3500: Dano := 30000;
      3501 .. 4000: Dano := 40000;
      else
      Dano := 90000; // Valor padrão caso o nível do mob não esteja nas faixas acima
    end;


    //Randomize;
    //inc(Dano, ((Random(1000) div 2) * 10));
    dnType := TDamageType.Normal;

    Self.Base.AttackParseForMobs(0, 0, @Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base, Dano, dnType, AddBuff, Packet.MobAnimation);
    Packet.dnType := dnType;
    Packet.DANO := Dano;


    if (Packet.DANO >= Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Character.CurrentScore.CurHP) then
    begin
      // Atualização do HP e status do jogador
      Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Character.CurrentScore.CurHP := 0;
      Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.IsDead := True;

      // Verifica se o jogador está em um grupo e obtém o número de membros restantes
      if (Servers[Self.Base.ChannelId].Players[Self.AttackerID].Party.Index > 0) then
      begin
        PartyMembers := Servers[Self.Base.ChannelId].Players[Self.AttackerID].Party.Members.Count;

        // Remove o jogador do grupo e redefine variáveis de calabouço
        Servers[Self.Base.ChannelId].Players[Self.AttackerID].Party.RemoveMember(Self.AttackerID);
        ...
        // Finaliza a instância se não houver mais membros no grupo
        if (PartyMembers <= 1) then
        begin
          with DungeonInstances[InstanceID] do
          begin
            ...
            DungeonID := 0;
            Dificult := 0;
            ZeroMemory(@MOBS, sizeof(MOBS));
          end;
          Servers[Self.Base.ChannelId].Players[Self.AttackerID].Party := nil;
        end;
      end;
    ...
    begin
      Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Character.CurrentScore.CurHP :=
        Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Character.CurrentScore.CurHP - Packet.DANO;
    end;
    ...
    on E: Exception do
      Logger.Write('Error at mob Attack Player [' + E.Message + '] b.e: ' +
        ...
        String(Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Character.Name) + ' t:' + DateTimeToStr(Now), TlogType.Error);
  end;
