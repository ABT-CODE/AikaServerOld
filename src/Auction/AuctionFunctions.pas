...
implementation
uses
  SysUtils, GlobalDefs, AnsiStrings, ItemFunctions, DateUtils, Log, SQL,Packethandlers;
  ...
  case SearchByName of
    ...
    257:
      begin
        ...
        'WHERE %s = %d AND (ItemLevel >= %d AND ItemLevel <= %d) ' +
        'AND (ReinforceLevel >= %d AND ReinforceLevel <= %d) ' +
        'ORDER BY ExpireDate DESC LIMIT 50',
...
begin
  ...
  TPacketHandlers.AddGoldToMarshalGuildChest(RegisterTax);
...
begin
  ...
  // Define valores padrão para ItemLevel e ItemReinforce se não forem válidos
  if (ItemLevel = 0) then
    ItemLevel := 1;

  // Checa se o item pode ser reforçado e seta um valor padrão caso não possa
  if (ItemList[Item.Index].Fortification) and (ItemList[Item.Index].Classe > 0) then
    ItemReinforce := Trunc(Item.Refi / 18)
  else
    ItemReinforce := 1;

  // Define o ItemType com base no item atual
  ItemType := ItemList[Item.Index].itemType;
  ...
  try
    // Insere na tabela principal `auction`
    QueryString :=
      ...
      [Player.Base.Character.CharIndex, ItemType, ItemLevel, ItemReinforce,
      FormatDateTime('yyyy-mm-dd hh:nn:ss', Now()), RegisterTime, SellingPrice, AuctionItemIndex]);
    ...
    // Após a inserção, busca o AuctionId recém-criado
    ...
    // Insere o item na tabela `vwauction_getactiveoffers`
    QueryString :=
      Format('INSERT INTO vwauction_getactiveoffers (AuctionId, CharacterId, ItemType, ItemLevel, ReinforceLevel, ' +
      'SellingPrice, ItemId, ItemLookId, IdentificableAddOns, EffectId_1, EffectValue_1, EffectId_2, EffectValue_2, ' +
      'EffectId_3, EffectValue_3, DurabilityMin, DurabilityMax, Amount_Reinforce, ItemTime, ExpireDate) ' +
      'VALUES (%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, "%s");',
      [AuctionOfferIndex, Player.Base.Character.CharIndex, ItemType, ItemLevel, ItemReinforce,
      SellingPrice, Item.Index, Item.APP, Item.Identific,
      ...
      FormatDateTime('yyyy-mm-dd hh:nn:ss', Now() + RegisterTime)]);
    ...
    begin
      ...
      Logger.Write('Erro ao registrar oferta no banco de dados: ' + E.Message, TlogType.Error);
      Result := False;
    end;
  end;
...
class function TAuctionFunctions.CancelItemOffer(var Player: TPlayer; AuctionOfferId: UInt64): Boolean;
var
  QueryString: string;
  Item: TItem;
  SQLComp: TQuery;
begin
  ...
  try
    // Primeiro, recuperar os dados do item
    QueryString := Format('SELECT ItemId, ItemLookId, IdentificableAddOns, EffectId_1, ' +
      'EffectId_2, EffectId_3, EffectValue_1, EffectValue_2, EffectValue_3, ' +
      'DurabilityMin, DurabilityMax, Amount_Reinforce, ItemTime ' +
      'FROM vwauction_getactiveoffers WHERE AuctionId = %d', [AuctionOfferId]);

    SQLComp.SetQuery(QueryString);
    SQLComp.Run();

    // Verifica se o item existe no leilão
    if (SQLComp.Query.RecordCount = 0) then
    begin
      SQLComp.Destroy;
      Result := False;
    ...
    // Preencher as informações do item para ser retornado
    ...
    Item.Identific := SQLComp.Query.FieldByName('IdentificableAddOns').AsInteger;
    ...
    Item.Effects.Value[0] := SQLComp.Query.FieldByName('EffectValue_1').AsInteger;
    Item.Effects.Value[1] := SQLComp.Query.FieldByName('EffectValue_2').AsInteger;
    Item.Effects.Value[2] := SQLComp.Query.FieldByName('EffectValue_3').AsInteger;
    ...
    // Remover da tabela de leilão
    QueryString := Format('UPDATE auction SET Active = 0 WHERE AuctionId = %d', [AuctionOfferId]);
    SQLComp.Query.Connection.StartTransaction;
    SQLComp.SetQuery(QueryString);
    SQLComp.Run(False);
    SQLComp.Query.Connection.Commit;
    ...
    // Remover da tabela `vwauction_getactiveoffers`
    QueryString := Format('DELETE FROM vwauction_getactiveoffers WHERE AuctionId = %d', [AuctionOfferId]);
    SQLComp.Query.Connection.StartTransaction;
    SQLComp.SetQuery(QueryString);
    SQLComp.Run(False);
    SQLComp.Query.Connection.Commit;
    ...
    // Devolver o item ao inventário do jogador
    ...
    on E: Exception do
    begin
      Logger.Write('Erro ao cancelar item no leilão: ' + E.Message, TlogType.Error);
      SQLComp.Destroy;
      Result := False;
      Exit;
    end;
...
class function TAuctionFunctions.RequestBuyItem(var Player: TPlayer; AuctionOfferId: UInt64): Boolean;
var
  QueryString: string;
...
begin
  ...
  AcquisitionMailId := 0;
  try
    // Registrar a aquisição para o comprador
    ...
    // Registrar a aquisição para o vendedor
    ...
    if not(SQLComp.Query.Connection.Connected) then
    begin
      Logger.Write('Falha de conexão individual com mysql.[RequestBuyItem]', TlogType.Warnings);
      SQLComp.Destroy;
      Exit;
    end;

    // Transferir o item do leilão para o correio do comprador
    QueryString := Format('INSERT INTO mails_items (mail_id, slot, item_id, app, identific, effect1_index, effect1_value, ' +
    ...
    // Remover o item da tabela `auction` (tabela principal de leilão)
    QueryString := Format('UPDATE auction SET Active=0 WHERE AuctionId=%d;', [AuctionOfferId]);
    SQLComp.SetQuery(QueryString);
    SQLComp.Query.Connection.StartTransaction;
    SQLComp.Run(False);
    ...
    // Remover o item da tabela `vwauction_getactiveoffers` (tabela de visualização)
    QueryString := Format('DELETE FROM vwauction_getactiveoffers WHERE AuctionId=%d;', [AuctionOfferId]);
    SQLComp.SetQuery(QueryString);
    SQLComp.Query.Connection.StartTransaction;
    SQLComp.Run(False);
    ...
    // Obter o valor da venda do item
    ...
    // Subtrair o valor da compra do ouro do jogador
    Player.DecGold(SellingPrice);

    // Confirmar a compra para o jogador
    Self.SendBuyResult(Player, AuctionOfferId);
    Player.SendClientMessage('O item comprado será entregue por correio');
  except
    on E: Exception do
    begin
      Logger.Write('Erro ao processar a compra no leilão: ' + E.Message, TlogType.Error);
      SQLComp.Destroy;
      Result := False;
      Exit;
    end;
