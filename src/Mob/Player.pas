...
begin
  ...
  Result := Byte(Sum mod 256); // Checksum simples
end;
...
begin
  ...
  try
    ...
    // Epoch := EncodeDateTime(2024, 1, 1, 0, 0, 0, 0); // DateTime do Epoch
    // LastTokenCreationTime := Epoch + (PlayerSQLComp.Query.FieldByName('last_token_creation_time').AsInteger / SecsPerDay);
    ...
    DateTimeValue := UnixToDateTime
      (PlayerSQLComp.Query.FieldByName('premium_time').AsInteger);
...
begin
  ...
  try
    ...
    begin
      ...
      Self.Account.Characters[Slot].LastPos.X := 3450;
        SQLComp.Query.FieldByName('posx').AsSingle;
      Self.Account.Characters[Slot].LastPos.Y := 688;
      ...
      Self.Account.CharactersDeleteTime[Slot] := ShortString(
  DateTimeToStr(
    SQLComp.Query.FieldByName('delete_time').AsLargeInt / 86400 + EncodeDate(1970, 1, 1)
  )
);
...
begin
  ...
  begin
    ...
    if (Self.Account.Characters[CharID].Base.GuildIndex > 900) or
   (Self.Account.Characters[CharID].Base.GuildIndex = 0) then
    begin
      var QueryGuild := TQuery.Create(AnsiString(MYSQL_SERVER), MYSQL_PORT,
        AnsiString(MYSQL_USERNAME), AnsiString(MYSQL_PASSWORD),
        AnsiString(MYSQL_DATABASE));
      try
        // Consulta o valor original da guilda na tabela playeroriginaldata
        QueryGuild.SetQuery(
          'SELECT OriginalGuildIndex FROM playeroriginaldata ' +
          'WHERE AccountID = :AccountID AND CharacterID = :CharacterID');
        QueryGuild.AddParameter2('AccountID', Self.Account.Characters[CharID].Index); // ID do personagem (characters.id)
        QueryGuild.AddParameter2('CharacterID', Self.Account.Header.AccountId); // owner_accid (characters.owner_accid)
        QueryGuild.Run;

        if not QueryGuild.Query.Eof then
        begin
          // Atualiza o GuildIndex na memória com o valor original
          Self.Account.Characters[CharID].Base.GuildIndex :=
            QueryGuild.Query.FieldByName('OriginalGuildIndex').AsInteger;

          // Atualiza o GuildIndex no banco de dados (tabela characters)
          var UpdateQuery := TQuery.Create(AnsiString(MYSQL_SERVER), MYSQL_PORT,
            AnsiString(MYSQL_USERNAME), AnsiString(MYSQL_PASSWORD),
            AnsiString(MYSQL_DATABASE));
          try
            UpdateQuery.SetQuery(
              'UPDATE characters SET guildindex = :OriginalGuildIndex ' +
              'WHERE id = :CharacterID');
            UpdateQuery.AddParameter2('OriginalGuildIndex', Self.Account.Characters[CharID].Base.GuildIndex);
            UpdateQuery.AddParameter2('CharacterID', Self.Account.Characters[CharID].Index);
            UpdateQuery.Run(False);
          finally
            UpdateQuery.Free;
          end;

          // Log para registro
          Logger.Write(Format('GuildIndex restaurado para o personagem %d. Novo GuildIndex: %d.',
            [Self.Account.Characters[CharID].Index, Self.Account.Characters[CharID].Base.GuildIndex]), TLogType.Error);
        end;
      finally
        QueryGuild.Free;
      end;
    ...
    { Buffs }
           try
        // Se desejar iniciar uma transação, descomente:
        // Self.PlayerSQL.MySQL.StartTransaction;

        MySQLComp.SetQuery(
          'SELECT buff_index, buff_time FROM buffs ' +
          'WHERE owner_charid = ' + ID.ToString + ' LIMIT 60'
        );
        // PlayerSQL.AddParameter2('powner_charid', ID);
        ...
        // Variável para identificar se encontrou 9122 ou 9123 e 9126
        var FoundBuff: Boolean := False;

        // Percorre até o máximo de 60 resultados
        for i := 0 to (ItemAmount - 1) do
        begin
          // Ignora caso buff_index seja zero
          ...
          // Copia buff_index e buff_time para a estrutura do personagem
          ...
          Self.Account.Characters[CharID].Buffs[i].CreationTime :=
            UnixToDateTime(
              StrToInt64(MySQLComp.Query.FieldByName('buff_time').AsString)
            );

          // Verifica se é buff 9122 ou 9123
          if (Self.Account.Characters[CharID].Buffs[i].Index = 9122) or
             (Self.Account.Characters[CharID].Buffs[i].Index = 9126) or
             (Self.Account.Characters[CharID].Buffs[i].Index = 9123) then
          begin
            FoundBuff := True;
          end;
        ...
        // Se não encontrou nenhum dos buffs 9122 ou 9123, atualiza altura/tronco
        if not FoundBuff then
        begin
          var QUpdate: TQuery := TQuery.Create(
          ...
          try
            if (QUpdate.Query.Connection.Connected) then
            begin
              QUpdate.SetQuery(
                'UPDATE characters SET altura = 7, tronco = 119 ' +
        ...
        // Se quiser confirmar a transação, descomente:
        // Self.PlayerSQL.MySQL.Commit;

      except
        on E: Exception do
        begin
          // Caso haja erro, trate aqui
    ...
    {try
      ...
      // Self.PlayerSQL.MySQL.StartTransaction;
       MySQLComp.SetQuery('SELECT buff_index, buff_time FROM buffs WHERE owner_charid = ' + ID.ToString + ' LIMIT 60');
       // PlayerSQL.AddParameter2('powner_charid', ID);
       MySQLComp.Run();
       ItemAmount := MySQLComp.Query.RecordCount;
       MySQLComp.Query.First;
...
begin
  ...
  try
    ...
    PlayerSQLComp.SetQuery('UPDATE accounts SET isactive=' + is_active.ToString
      + ', ' + 'premium_time="' +
      IntToStr(DateTimeToUnix(Self.Account.Header.PremiumTime)) +
...
var
  ...
  SQLComp: TQuery;
const
  MAX_SLOTS = 3; // Define o número máximo de slots (0, 1, 2)
begin
  Result := False;
  ...
  if not(SQLComp.Query.Connection.Connected) then
  begin
    Logger.Write('Falha de conexão individual com mysql.[DeleteCharacter]',
      TlogType.Warnings);
    Logger.Write('PERSONAL MYSQL FAILED LOAD.[DeleteCharacter]', TlogType.Error);
    SQLComp.Destroy;
    Exit;
  end;
  ...
  SQLComp.Query.Connection.StartTransaction;
  try
    // Verificar se o personagem no slot existe
    if Self.Account.Characters[CharID].Base.Name = '' then
    begin
      Logger.Write('Tentativa de excluir personagem inexistente no slot.', TLogType.Warnings);
      Exit;
    end;

    // Obter o ID do personagem a ser excluído
    ID := Self.Account.Characters[CharID].Index;

    // Excluir o personagem da tabela 'characters'
    SQLComp.SetQuery(format('DELETE FROM characters WHERE id=%d', [ID]));
    SQLComp.Run(false);

    // Reorganizar os slots na memória e no banco de dados
    for i := CharID to MAX_SLOTS - 2 do
    begin
      if Self.Account.Characters[i + 1].Index > 0 then
      begin
        // Atualizar o slot no banco de dados
        SQLComp.SetQuery('UPDATE characters SET slot = :newSlot WHERE id = :charId');
        SQLComp.Query.Params[0].AsInteger := i;                // Novo slot
        SQLComp.Query.Params[1].AsInteger := Self.Account.Characters[i + 1].Index; // ID do personagem
        SQLComp.Run(false);

        // Reorganizar os dados na memória
        Self.Account.Characters[i] := Self.Account.Characters[i + 1];
      end;
    end;

    // Limpar o último slot na memória
    ZeroMemory(@Self.Account.Characters[MAX_SLOTS - 1], sizeof(TCharacterDB));

    // Atualizar o banco para limpar o último slot, se necessário
    SQLComp.SetQuery('DELETE FROM characters WHERE slot = :slot AND owner_id = :ownerId');
    SQLComp.Query.Params[0].AsInteger := MAX_SLOTS - 1;
    SQLComp.Query.Params[1].AsInteger := Self.Account.Header.AccountID; // Substitua por como você armazena o ID do proprietário
    SQLComp.Run(false);

    SQLComp.Query.Connection.Commit;

    // Atualizar o cliente para exibir os slots reorganizados
    Self.SendCharList;
...
begin
  ...
  try
    ...
    begin
      try
        MySQLComp.SetQuery('SELECT lastlogin from characters where id =' +
          ID.ToString);
        MySQLComp.Run();
        ...
        begin
          ...
          begin
            if (HourOf(TFunctions.UNIXTimeToDateTimeFAST(MySQLComp.Query.Fields[0].AsInteger)) <> HourOf(Now))
            then
            begin

              TFunctions.SaveAccountFile(Self.Account,
                String(Self.Account.Header.Username) + '_' +
                FormatDateTime('dd_mm_yyyy_hh_nn_ss', Now),
                String(Self.Account.Header.Username));
            end;
          end
          else
          begin
            Logger.Write('SaveInGame[2]', TLogType.Error);
            TFunctions.SaveAccountFile(Self.Account,
              String(Self.Account.Header.Username) + '_' + FormatDateTime('dd_mm_yyyy_hh_nn_ss', Now),
              String(Self.Account.Header.Username));
          end;
        end;
      except
        ...
        begin
          TFunctions.SaveAccountFile(Self.Account,
            ...
            FormatDateTime('dd_mm_yyyy_hh_nn_ss', Now),
            String(Self.Account.Header.Username));
          Logger.Write(E.Message + ' erro ao executar querie para backup.',
            TlogType.Error);
        end;
      end;
    end;
    ...
    MySQLComp.SetQuery('UPDATE accounts SET isactive=' + IntToStr(ifthen(Self.Account.Header.IsActive, 1, 0)) +
                                            ', nation=' + Integer(Self.Account.Header.Nation).ToString +
                                            ', storage_gold=' + Self.Account.Header.Storage.Gold.ToString +
                                            ', cash=' + Self.Account.Header.CashInventory.Cash.ToString +
                                            ', account_status=' + Self.Account.Header.AccountStatus.ToString +
                                            ', ban_days=' + Self.Account.Header.BanDays.ToString +
                                            ' WHERE id=' + Self.Account.Header.AccountId.ToString + ';');
  ...
  try
    ...
    MySQLComp.SetQuery('UPDATE characters SET name="' +
      ...
      TFunctions.DateTimeToUNIXTimeFAST(Now).ToString() + '", playerkill=' +
    ...
    begin
      ...
      begin
        VarQuery := VarQuery + ', (' + Self.Base.PlayerCharacter.Buffs[i].
          Index.ToString + ', "' + TFunctions.DateTimeToUNIXTimeFAST(Self.Base.PlayerCharacter.Buffs
          [i].CreationTime).ToString() + '", ' + ID.ToString + ')';
      end
      else
      begin
        VarQuery := VarQuery + '(' + Self.Base.PlayerCharacter.Buffs[i].
...
begin
  ...
  begin
    ...
    begin
      ...
      if ((Self.Account.Header.Pran1.Equip[5].Index <> 0) and
      (ItemList[Self.Account.Header.Pran1.Equip[5].Index].Classe <> 102) ) then
      begin
  ...
  begin
    ...
    begin
      ...
      if ((Self.Account.Header.Pran2.Equip[5].Index <> 0) and
      (ItemList[Self.Account.Header.Pran2.Equip[5].Index].Classe <> 102) ) then
      begin
...
var
  ...
  i: Integer;
begin
  // Verificação para garantir que apenas o personagem no slot 0 pode ativar o Auxílio Poderoso
  if Self.SelectedCharacterIndex < 0 then
  begin
    Self.SendClientMessage('O Auxílio Poderoso só pode ser utilizado no personagem do slot 1.');
    Exit;
  end;

  // Verificação para garantir que o personagem no slot 0 tem o item 8250 no inventário
  ItemFound := False;
  for i := 0 to 59 do
  begin
  ...
  begin
    Self.SendClientMessage('Você precisa do Ax Poderoso no inventário para ativar o Auxílio .');
    Exit;
  end;
  ...
  if ((Self.Account.Header.PremiumTime <> 0) and
    (DateTimeToStr(Self.Account.Header.PremiumTime) <> '01/01/2020 00:00:00')) then
  begin
...
begin
  ...
  begin
    if not(DateTimeToStr(Self.Account.Header.PremiumTime) = '01/01/2020 00:00:00') then
    begin
    ...
    begin
       if (Self.Base.Character.Inventory[i].Index = 8250) then
    begin
      // Verificação para garantir que apenas o personagem no slot 0 com item 8250 pode ativar o Auxílio Poderoso
      if (Self.SelectedCharacterIndex <> 0) then
      begin
        Self.SendClientMessage('O Auxílio Poderoso só pode ser utilizado no personagem do slot 1.');
      end
      ...
      begin
        // Atualiza a data de expiração do Auxílio Poderoso para 30 dias a partir de agora
        Self.Account.Header.PremiumTime := IncDay(Now, 30);

        // Enviar o status atualizado do auxílio ao jogador
        Self.SendClientMessage('Seu auxílio poderoso foi ativado.');
        Self.SendAccountStatus();
      ...
      if not(TItemFunctions.GetItemEquipSlot(Self.Base.Character.Inventory[i].Index) = 9) then
        Continue;
    ...
    begin
      ...
      begin
        ...
        if (Self.Account.Characters[J].Base.CharIndex = Self.Base.Character.CharIndex) then
          Continue;
        ...
        begin
          ...
          begin
            if (DateTimeToStr(Self.Account.Header.PremiumTime) = '01/01/2020 00:00:00') then
            begin
              Self.SendClientMessage('Seu auxílio poderoso foi ativado. Você tem 30 dias Premium.');
              Self.Account.Header.PremiumTime := Now;
              Self.Account.Header.PremiumTime := IncDay(Self.Account.Header.PremiumTime, 30);
              Self.SendAccountStatus();
              AlreadyActivatedAux := True;
              IsAuxilyUser := True;
            end
  ...
  begin
    if (Self.Base.PlayerCharacter.Buffs[i].Index = 0) then
      Continue;
    Self.Base.AddBuffWhenEntering(Self.Base.PlayerCharacter.Buffs[i].Index, Self.Base.PlayerCharacter.Buffs[i].CreationTime);
  end;
  Self.Status := Playing;
  ...
  begin
    // corrigindo os atributos da montaria
    Self.Base.SendRefreshItemSlot(EQUIP_TYPE, 9, Self.Base.Character.Equip[9], false);
  end;
  ...
  begin
    if (Self.Account.Header.Pran1.ItemID = Self.Character.Base.Equip[10].Identific) then
    begin
    ...
    if (Self.Account.Header.Pran2.ItemID = Self.Character.Base.Equip[10].Identific) then
    begin
...
begin
  try
    ...
    // Addbuff Bandeira Marechal leopold
...
begin
  ...
  case ExpType of
    EXP_TYPE_NORMAL:
      begin
        // Determinar o divisor de XP baseado no nível
        if (Self.Character.Base.Level >= 85) and (Self.Character.Base.Level < 90) then
          ExpDivisor := 5
        else if (Self.Character.Base.Level >= 90) and (Self.Character.Base.Level < 95) then
          ExpDivisor := 30
        else if (Self.Character.Base.Level >= 95) and (Self.Character.Base.Level < 98) then
          ExpDivisor := 50
        else if (Self.Character.Base.Level >= 98) and (Self.Character.Base.Level <= 99) then
          ExpDivisor := 90
        else
          ExpDivisor := 1; // XP normal antes do nível 85

        // Aplicar a divisão antes de adicionar XP
        Value := Value div ExpDivisor;

        // Adicionar XP reduzida ao personagem
    ...
    EXP_TYPE_MOB:
      begin
        ...
        // Aplicar bônus de relíquia antes da redução
       ...
       //   Inc(NewExp, Servers[Self.ChannelIndex].ReliqEffect[EF_RELIQUE_PER_EXP] * (NewExp div 100)) ;
        ...
        // Definir o divisor de XP baseado no nível
        if (Self.Character.Base.Level >= 85) and (Self.Character.Base.Level < 90) then
          ExpDivisor := 5
        else if (Self.Character.Base.Level >= 90) and (Self.Character.Base.Level < 95) then
          ExpDivisor := 30
        ...
        // Aplicar a redução ao XP
        NewExp := NewExp div ExpDivisor;
        ExpPreReliq := NewExp; // Agora ambos têm o mesmo valor reduzido
    ...
    EXP_TYPE_QUEST:
      begin
        // Aqui pode-se aplicar outra lógica para missões se necessário.
      end;
  end;

  // Verifica se o XP acumulado já atingiu o necessário para subir de nível
  if (Self.Character.Base.Exp + NewExp >= ExpList[Self.Character.Base.Level]) then
  begin
    // Bloqueia o ganho de XP se o jogador precisar mudar de classe
    if (Self.Character.Base.Level = 51) and not (Self.Character.Base.ClassInfo in [2, 12, 22, 32, 42, 52]) then
    begin
      Self.SendClientMessage('Você precisa mudar para a Segunda Classe antes de continuar ganhando experiência!');
      Exit; // Impede que o XP seja adicionado
    end;

    if (Self.Character.Base.Level = 90) and not (Self.Character.Base.ClassInfo in [3, 13, 23, 33, 43, 53]) then
    begin
      Self.SendClientMessage('Você precisa mudar para a Terceira Classe antes de continuar ganhando experiência!');
      Exit; // Impede que o XP seja adicionado
    end;
  end;

  // Se passou na verificação, adiciona o XP ao personagem
  Inc(Self.Character.Base.Exp, NewExp);
  Result := NewExp;
  ...
  // Impedir que XP ultrapasse o máximo do nível
  ...
  // Atualizar a interface do jogador
  Self.Base.SendRefreshLevel;
end;
...
begin
  ...
  // Se o jogador estiver no nível 99, não ganha mais XP
  if (Self.Character.Base.Level = 99) then
  begin
    Exit;
  end;

  // Diferença de XP necessária para o próximo nível
  DifLevel := ExpList[Self.Character.Base.Level] - ExpList[Self.Character.Base.Level - 1] ;

  // Calcula o XP baseado no percentual
  Exp := Round(DifLevel * Perc) ;


  // Adiciona XP reduzida ao personagem
  ...
  // Impede que XP ultrapasse o limite no nível máximo
  if (Self.Character.Base.Level = LEVEL_CAP) and
    (Self.Character.Base.Exp > ExpList[Self.Character.Base.Level]) then
    Self.Character.Base.Exp := ExpList[Self.Character.Base.Level];
...
begin
  ...
  begin
    ...
    // Pontos adicionais para níveis acima de 85 até 99
    if (Self.Character.Base.Level >= 85) and (Self.Character.Base.Level <= 99) then
    begin
      Inc(Self.Character.Base.CurrentScore.SkillPoint, SKILL_POINT_EXTRA);
      Inc(Self.Character.Base.CurrentScore.Status, STATUS_POINT_EXTRA);
    end;
...
var
  ...
  begin
      ...
      SQLComp.SetQuery('SELECT * FROM titles WHERE owner_charid=:onwer_id AND title_index=:title_id');
      SQLComp.AddParameter2('owner_charid',Self.Base.Character.CharIndex);
      SQLComp.AddParameter2('title_id',TitleID);
      SQLComp.Run();

      count := SQLComp.Query.RecordCount;
...
begin
  ...
  Self.SendClientMessage('Você teve o título [' +
    AnsiString(Titles[TitleID].TitleLevel[0].TitleName) + '] deletado Você será desconectado em 10 segundos.');
  SQLComp.Destroy;
  ...
  begin
    ...
    begin
      ...
      begin
       ...
       if (i = 91) then
      begin
        Inc(Result, SKILL_POINT_X + 10);
      end;

       if (i = 98) then
      begin
        Inc(Result, SKILL_POINT_X +12 );
      end;
    end;
  end;
end;
      ...
      case SkillData[Skill].Index of
        9: // fortitude WR atk fis 2 e acerto 4
          begin
            ...
            begin // para entrar no jogo
              Self.Base.IncreasseMobAbility(EF_DAMAGE1, (Level * 2)*250);
              Self.Base.IncreasseMobAbility(EF_HIT, (Level * 4));
            end
            else if (Mode = 1) then
            begin // para reiniciar todas as habilidades
              Self.Base.DecreasseMobAbility(EF_DAMAGE1, (Level * 2)*250);
        ...
        10: // corpo draconiano WR hp 145 e 5% de recuperação de hp
          begin
            ...
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_HP, (Level * 145)*5);
              Self.Base.IncreasseMobAbility(EF_REGENHP,
                (Level * ((helper div 100) * 5))*2);
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_HP, (Level * 145)*5);
        ...
        23: // Instinto de batalha WR Reduz dano recebido em area 30%+(2%*lvl) e critico 1
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_RESISTANCE6, (50 + (Level * 2)*50));
              Self.Base.IncreasseMobAbility(EF_REDUCE_AOE, (50 + (Level * 2)*150));
              Self.Base.IncreasseMobAbility(EF_CRITICAL, Level);
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_RESISTANCE6, (50 + (Level * 2)* 50));
              Self.Base.IncreasseMobAbility(EF_REDUCE_AOE, (50 + (Level * 2)*150));
        ...
        149: // Defesa Automatica TP 10%+2*lvl de diminuir os danos em 5%+1*lvl
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_GUARD_RATE, (10 + (Level * 4)));
              Self.Base.IncreasseMobAbility(EF_GUARD, (10 + Level));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_GUARD_RATE, (10 + (Level * 4)));
              Self.Base.DecreasseMobAbility(EF_GUARD, (10 + Level));
            end;
          end;
        ...
        152: // Ultimato att skill damage 50 + 50*lvl e velo 1 + lvl
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_RUNSPEED, (2 + (Level)));
              Self.Base.IncreasseMobAbility(EF_SKILL_DAMAGE,
                (150 + (Level * 150)));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_RUNSPEED, (2 + (Level)));
        ...
        81: // Olhar Penetrante DG crit 1 e dano crit 3
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_CRITICAL, Level +25);
              Self.Base.IncreasseMobAbility(EF_CRITICAL_POWER, (Level * 10));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_CRITICAL, Level+25);
              Self.Base.DecreasseMobAbility(EF_CRITICAL_POWER, (Level * 10));
            end;
          end;
        82: // Movimento Gracioso DG esquiva 2 + lvl
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_PARRY, (20 + Level));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_PARRY, (20 + Level));
            end;
          end;
        155: // Vento Cortante DG res lentidao 5 e res paralisia 3
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_IM_RUNSPEED, (Level * 15));
              Self.Base.IncreasseMobAbility(EF_IM_SKILL_IMMOVABLE, (Level * 5));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_IM_RUNSPEED, (Level * 15));
        ...
        105: // Tempestade de Mana FC
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_DAMAGE2, (Level * 3000));
              Self.Base.IncreasseMobAbility(EF_PRAN_REQUIRE_MP, (Level * 10));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_DAMAGE2, (Level * 3000));
              Self.Base.DecreasseMobAbility(EF_PRAN_REQUIRE_MP, (Level * 10));
            end;
          end;
        106: // Harmonia de Mana FC
          begin
            ...
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_MP, (20000 * Level));
              Self.Base.IncreasseMobAbility(EF_REGENMP, (1500 * Level));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_MP, (20000 * Level));
              Self.Base.DecreasseMobAbility(EF_REGENMP, (1500 * Level));
            end;
          end;
        158: // Afinidade Negra FC res silence 5 e terror 3
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_IM_SILENCE1, (Level * 6));
              Self.Base.IncreasseMobAbility(EF_IM_FEAR, (Level * 4));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_IM_SILENCE1, (Level * 6));
              Self.Base.DecreasseMobAbility(EF_IM_FEAR, (Level * 4));
            end;
          end;
        ...
        129: // Ativação divina CL cura 120 diminui mp consumido em 10% cura
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_SKILL_DAMAGE6, (Level * 120)*200);
              Self.Base.IncreasseMobAbility(EF_MPCURE, (8 + (2 * Level)));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_SKILL_DAMAGE6, (Level * 120)*200);
        ...
        161: // Vontade Inabalável CL res paralisia 15 choque 13
          begin
            if (Mode = 0) then
            begin
              Self.Base.IncreasseMobAbility(EF_IM_SKILL_IMMOVABLE, (Level * 19));
              Self.Base.IncreasseMobAbility(EF_IM_SKILL_SHOCK, (Level * 17));
            end
            else if (Mode = 1) then
            begin
              Self.Base.DecreasseMobAbility(EF_IM_SKILL_IMMOVABLE, (Level * 19));
              Self.Base.DecreasseMobAbility(EF_IM_SKILL_SHOCK, (Level * 17));
            end;
          end;
...
begin
  case SkillData[SkillIndex].Index of
    ...
    58: // Poder Critico Att critical power 5% + 5*lvl
      begin
        if (SkillData[SkillIDLevel].Level = 1) then
          Self.Base.IncreasseMobAbility(EF_CRITICAL_POWER, (5 + 5* 5))
        else
    ...
    81: // Olhar Penetrante DG crit 1 e dano crit 3
      begin
        Self.Base.IncreasseMobAbility(EF_CRITICAL, 1 + 25);
        Self.Base.IncreasseMobAbility(EF_CRITICAL_POWER, 3 + 25);
      end;
    82: // Movimento Gracioso DG esquiva 2 + lvl
      begin
        if (SkillData[SkillIDLevel].Level = 1) then
...
begin
  try
    ...
    ZeroMemory(@Packet, SizeOf(Packet));
    // Logger.Write('2.7', TLogType.Error);
    ...
    //Logger.Write('2.8', TLogType.Error);
    // Fill the packet with the required data
    PWORD(@Packet[6])^ := $152;
...
begin
  ...
  try
    ...
    begin
      // Define a data de última atualização como 25 horas atrás e converte para Unix
    ...
    // Converte o timestamp Unix armazenado para TDateTime
    LastEventUnix := StrToInt64(TimeInString);
    LastReceived := UnixToDateTime(LastEventUnix);
