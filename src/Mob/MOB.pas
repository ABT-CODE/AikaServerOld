...
type
  ...
  TMobSa = record // static informations
    ...
    FisAtk, MagAtk, FisDef, MagDef: integer; // mob attributes
              ...
              begin
                ...
                begin
                  if (Servers[Self.Base.ChannelId].Players[Self.AttackerID]
                      .Base.BuffExistsByIndex(36)) and (Servers[Self.Base.ChannelId].Players[Self.AttackerID]
                      .Base.BuffExistsByIndex(365)) then
                  begin
...
begin
  ...
  try
    ...
    if (Self.AttackerID > 0) and (Self.Base.IsDead = FALSE) and (Self.IsAttacked = TRUE) then
    begin
      ...
      if (Self.isGuard) then
      begin
        // Move-se em direção à posição do atacante, se for um guard
        Self.XPositionDif := Round(Self.CurrentPos.X - Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Neighbors[Rand].pos.X);
        Self.XPositionDif := abs(Self.XPositionDif);
        if (Self.XPositionDif >= 3) then
        begin
          if (Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Neighbors[Rand].pos.X < Self.CurrentPos.X) then
            Self.CurrentPos.X := Self.CurrentPos.X - 2
          else
            Self.CurrentPos.X := Self.CurrentPos.X + 2;
        end;

        Self.YPositionDif := Round(Self.CurrentPos.Y - Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Neighbors[Rand].pos.Y);
        Self.YPositionDif := abs(Self.YPositionDif);
        if (Self.YPositionDif >= 3) then
        begin
          if (Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Neighbors[Rand].pos.Y < Self.CurrentPos.Y) then
      ...
      begin
        // Move-se em direção a uma posição aleatória próxima ao atacante
        Self.XPositionDif := Round(Self.CurrentPos.X - Servers[Self.Base.ChannelId].Players[Self.AttackerID].Base.Neighbors[Rand].pos.X);
        Self.XPositionDif := abs(Self.XPositionDif);
        if (Self.XPositionDif >= 3) then
        begin
    ...
    if (Self.IsAttacked = TRUE) and (Self.CurrentPos.Distance(Self.InitPos) >= 40) then
    begin
      // Verifica se o mob não possui habilidades que impedem movimentação
      if (Self.Base.GetMobAbility(EF_SKILL_STUN) <> 0) or
         (Self.Base.GetMobAbility(EF_SKILL_IMMOVABLE) <> 0) or
      ...
      begin
        if (Servers[Self.Base.ChannelId].Players[i].Status >= Playing) and
           ...
           (Servers[Self.Base.ChannelId].Players[i].Base.PlayerCharacter.LastPos.InRange(Self.InitPos, 40)) then
        begin
          OPP := @Servers[Self.Base.ChannelId].Players[i];
          Break;
        end;
...
begin
  try
    ...
    if (DANO < 0) then
     ...
     // **Multiplicação do dano final antes de ser enviado**
      if (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel >= 361) and
     (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel <= 400) then
      DANO := Round(DANO * 1) // Mobs de nível 300+ aplicam 150x o dano


     else if (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel >= 351) and
     (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel <= 360) then
      DANO := Round(DANO * 3.0) // Mobs de nível 300+ aplicam 150x o dano
     else if (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel >=301)
     and (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel <= 350) then
      DANO := Round(DANO * 2.0) // Mobs de nível 300+ aplicam 150x o dano
    else  if (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel >= 201)
    and (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel <= 300) then
      DANO := Round(DANO * 1.0) // Mobs de nível 300+ aplicam 30x o dano
    else if (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel >= 199)
    and (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel <= 200) then
      DANO := Round(DANO * 0.9) // Mobs de nível 200+ aplicam 20x o dano
    else if (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel >= 99) and
    (Servers[Self.Base.ChannelId].MOBS.TMobS[Self.Base.Mobid].MobLevel <= 198) then
      DANO := Round(DANO * 0.6) // Mobs de nível ate 99 aplicam 10x o dano
    else
      DANO := Round(DANO * 0.5); // Outros mobs aplicam 15.5x o dano
...
begin
  ...
  if (ResultDamage < 0) or (ResultDamage > 2000000) then
    ResultDamage := 1;
  Result := ResultDamage;
end;
...
begin
  try
   ...
   // begin
    ...
    //end;
  except   // alteração dia 06/08/2024
    on E: Exception do
    begin
