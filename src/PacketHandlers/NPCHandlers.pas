...
type
  ...
  public
    ...
    class function LilolaBuff(var Player: TPlayer; NpcId: WORD): Boolean;
     class function marechalbuff(var Player: TPlayer; NpcId: WORD): Boolean;
...
begin
  ...
  case NPC.Base.ClientId of
    ...
    2093: //teleport de regenchain
      begin
        NPC.NPCFile.Header.Options[0] := 82; //Dg Caverna Hecla inferior
        NPC.NPCFile.Header.Options[1] := 81; //Dg Caverna Hecla superior
        NPC.NPCFile.Header.Options[2] := 14; //Dg Caverna Hecla superior
    ...
    2196: //teleport de basilan
      begin
        NPC.NPCFile.Header.Options[0] := 79; //Embate dos Destemidos Zona 1
        NPC.NPCFile.Header.Options[1] := 83; //Embate dos Destemidos Zona 2
        NPC.NPCFile.Header.Options[2] := 84; //Embate dos Destemidos Zona 3
        NPC.NPCFile.Header.Options[3] := 89; //Karmeon
        NPC.NPCFile.Header.Options[4] := 53; //Vale do Silencio
        NPC.NPCFile.Header.Options[5] := 60; //MT Aboel Leopold
        NPC.NPCFile.Header.Options[6] := 63; //DG Jardim Suspenso de Mirza
        NPC.NPCFile.Header.Options[7] := 64; //Leopold
        NPC.NPCFile.Header.Options[8] := 8; //Fechar
      ...
      2302: //DG Prosão Termes
      begin
        NPC.NPCFile.Header.Options[0] := 87; //Embate dos Destemidos Zona 1
       ...
       2298: //DG Prosão Termes
      begin
        NPC.NPCFile.Header.Options[0] := 88; //Sair da DG
        NPC.NPCFile.Header.Options[1] := 8; //Fechar
      ...
      2462: //DG Abismo +14
      begin
       NPC.NPCFile.Header.Options[0] := 51;
        NPC.NPCFile.Header.Options[1] := 8; //Fechar
       ...
       2461: //Sair DG
      begin
        NPC.NPCFile.Header.Options[0] := 88;
        NPC.NPCFile.Header.Options[1] := 8; //Fechar
      ...
      2340: //DG Leoponf +15
      begin
       NPC.NPCFile.Header.Options[0] := 61; //DG Leopldo +15
        NPC.NPCFile.Header.Options[1] := 8; //Fechar

      end;

       2344: // Sair DG Leoponf +15
      begin
        NPC.NPCFile.Header.Options[0] := 88;
        NPC.NPCFile.Header.Options[1] := 8; //Fechar
      ...
      2129: // Colliseu Tiamat
    begin

      // Verifica se o dia e horário estão dentro do permitido ou se o jogador possui o item 14200


            // Verifica se o jogador está dentro do range de 50 unidades da posição (3551, 2759)
      if Player.Base.PlayerCharacter.LastPos.InRange(TPosition.Create(2947, 1664), 3) then
      begin
        // Se estiver dentro do range, define a opção para 80 (acesso permitido)
        NPC.NPCFile.Header.Options[3] := 80;
      end
      else
      begin
        // Se não estiver dentro do range, exibe uma mensagem (ou outra ação desejada)
        Player.SendClientMessage('Você precisa estar mais próximo do NPC para acessar esta área.');
        exit
      end;
    end;
     ...
     2443: // Voltar tiamat deskeroa
          begin
            NPC.NPCFile.Header.Options[0] := 88;
           NPC.NPCFile.Header.Options[1] := 8; //Fechar
          end;

          2079: // Voltar tiamat destemidos
          begin
            NPC.NPCFile.Header.Options[0] := 88;
            NPC.NPCFile.Header.Options[1] := 8; //Fechar
        ...
        begin
         ...
         2103: // entrda oficial DG Hecla superior
        begin
          NPC.NPCFile.Header.Options[0] := 82;
          NPC.NPCFile.Header.Options[1] := 8; //Fechar
         ...
         2095: // entrda oficial DG Hecla inferior
        begin
          NPC.NPCFile.Header.Options[0] := 81;
          NPC.NPCFile.Header.Options[1] := 8; //Fechar


        end;

         2244: // sair oficial DG Hecla inferior
        begin
          NPC.NPCFile.Header.Options[0] := 88;
          NPC.NPCFile.Header.Options[1] := 8; //Fechar
         ...
         2197: // entrda MNina 1 lenfer
        begin
          NPC.NPCFile.Header.Options[0] := 14;
          NPC.NPCFile.Header.Options[1] := 8; //Fechar


        end;

        2242: // sair MNina 1 lenfer
        begin
          NPC.NPCFile.Header.Options[0] := 88;
          NPC.NPCFile.Header.Options[1] := 8; //Fechar
  ...
  begin
    if (NPC.NPCFile.Header.Options[i] > 0) and
      (NPC.NPCFile.Header.Options[i] < 90) then
    begin
...
begin
  ...
  case Option of
    ...
    67:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Karena', 64);
        Packet.Show := $FFffdb4d;
      end;
    68:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Deskeroa [2500 gold]', 64);
    ...
    78:
      ...
      79:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Embate dos Destemidos Zona 1', 64);
        Packet.Show := $FFffdb4d;
      end;

      80:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Battle Royale Wars', 64);
        Packet.Show := $FFffdb4d;
      end;

      82:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Caverna Hecla Inferior', 64);
        Packet.Show := $FFffdb4d;
      end;

      81:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Caverna Hecle Superior', 64);
        Packet.Show := $FFffdb4d;
      end;

      83:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Embate dos Destemidos Zona 2', 64);
        Packet.Show := $FFffdb4d;
      end;

      84:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Embate dos Destemidos Zona 3', 64);
        Packet.Show := $FFffdb4d;
      end;

      85:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Loja Especial Wars Zona 1 ', 64);
        Packet.Show := $FFffdb4d;
      end;
      86:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Loja Especial Wars Zona 2 ', 64);
        Packet.Show := $FFffdb4d;
      end;
      ...
      87:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'DG PVP Prisão Despero ', 64);
        Packet.Show := $FFffdb4d;
      end;

      88:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, 'Voltar Tiamat ', 64);
        Packet.Show := $FFffdb4d;
      end;

       89:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' Ir para Karmon ', 64);
        Packet.Show := $FFffdb4d;
      end;

       14:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' DG Mina 1 Lenfer ', 64);
        Packet.Show := $FFffdb4d;
      end;
       ...
       51:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' DG PVP do Abismo  ', 64);
        Packet.Show := $FFffdb4d;
      end;

        53:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' Vale do Silencio  ', 64);
        Packet.Show := $FFffdb4d;
      end;
        ...
        60:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' MT Aboel Leopold  ', 64);
        Packet.Show := $FFffdb4d;
      end;

       61:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' DG Caverna Lycan ', 64);
        Packet.Show := $FFffdb4d;
      end;

       63:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' DG Jardim Suspenso Mirza ', 64);
        Packet.Show := $FFffdb4d;
      end;

       64:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' Teleporte Leopold', 64);
        Packet.Show := $FFffdb4d;
      end;

       23:
      begin
        System.AnsiStrings.StrPLCopy(Packet.Name, ' Buff do Marechal', 64);
        Packet.Show := $FFffdb4d;
      end;
...
class function TNPCHandlers.LilolaBuff(var Player: TPlayer;
  NpcId: WORD): Boolean;
begin
  Player.Base.AddBuff(263, True,false, 3000); // Exemplo: buff leve
  Player.Base.AddBuff(201, True, False, 3000);
  Player.Base.AddBuff(340, True,false, 3000); // Exemplo: buff leve
  Player.Base.AddBuff(4955, True, False, 3000);
  Player.Base.AddBuff(5001, True,false, 3000); // Exemplo: buff leve
  Player.Base.AddBuff(5155, True, False, 3000);
  Result := True;
end;
class function TNPCHandlers.marechalbuff(var Player: TPlayer;
  NpcId: WORD): Boolean;
begin
//  Player.Base.AddBuff(7364, True);
                    ...
                    0:
                      begin
                        ...
                        begin
                          ...
                          Player.Base.Character.Equip[10].Index := 111;
                          Player.Base.Character.Equip[10].APP := 111;
                          Player.SendPranToWorld(0);
                          Player.SendPranSpawn(0);
                          Player.Base.SendRefreshItemSlot(EQUIP_TYPE, 10,
                            Player.Base.Character.Equip[10], False);
                    ...
                    1:
                      begin
                        ...
                        begin
                          ...
                          Inc(Player.Account.Header.Pran2.ClassPran);
                          Player.Account.Header.Pran2.Equip[0].Index := 111;
                          Player.Account.Header.Pran2.Equip[0].APP := 111;
                          Player.Base.Character.Equip[10].Index := 111;
                          Player.Base.Character.Equip[10].APP := 111;
                          Player.SendPranToWorld(1);
                          Player.SendPranSpawn(1);
...
class function TNPCHandlers.SignInCastle(var Player: TPlayer; NpcId: WORD): Boolean;
var
  xGuild: PGuild;
  i: Integer;
  EmptyAttackSlot: Byte;
begin
  // Verifica se o jogador possui o item 14200 no inventário
  var HasItem14200: Boolean := False;

  for  i := 0 to High(Player.Base.Character.Inventory) do
begin
  if Player.Base.Character.Inventory[i].Index = 14200 then
  begin
    HasItem14200 := True;
    Break; // Interrompe a verificação se encontrar o item
  end;
end;

// Verifica se o dia e horário estão dentro do permitido ou se o jogador possui o item 14200
if not (
  ((DayOfWeek(Now) = 2) and (HourOf(Now) >= 10) and (HourOf(Now) < 20)) or // Segunda-feira das 08:00 às 12:00
  ((DayOfWeek(Now) = 4) and (HourOf(Now) >= 10) and (HourOf(Now) < 20)) or // Quarta-feira das 08:00 às 12:00
  ((DayOfWeek(Now) = 6) and (HourOf(Now) >= 10) and (HourOf(Now) < 20))    // Sexta-feira das 08:00 às 12:00
) and not HasItem14200 then
begin
  Player.SendClientMessage
  ('Você só pode ee cadastrar para a Guerra de Castelos as segundas, quartas e sextas das 10:00 às 20:00.');
  Exit(False);
end;

    // Permissão concedida
    Player.SendClientMessage('Acesso ao castelo concedido.');
   // Exit(True);

  if(Player.Character.GuildSlot = 0) then
  begin
    Player.SendClientMessage('Voce tem que estar em uma legião para se cadastrar no castelo.');
    Exit(False);
  end;
  xGuild := @Guilds[Player.Character.GuildSlot];
  if(xGuild.GuildLeaderCharIndex <> Player.Base.Character.CharIndex) then
  begin
    Player.SendClientMessage('Voce deve ser o lider da legião para cadastrar na guerra.');
    Exit(False);
  end;
  if(xGuild.Ally.Leader <> xGuild.Index) then
  begin
    Player.SendClientMessage('Voce deve ser o lider da aliança para cadastrar na guerra.');
    Exit(False);
  end;
  for I := 0 to 3 do
  begin
    ...
    begin
      if(String(Nations[xGuild.Nation-1].Cerco.Atacantes[i].LordMarechal) = String(xGuild.Name)) then
      begin
        Player.SendClientMessage('Você ja esta com a sua aliança cadastrada na guerra.');
        Exit(False);
      end;
    end
    else
    ...
    begin
      if(String(Nations[xGuild.Nation-1].Cerco.Defensoras.LordMarechal) = String(xGuild.Name)) then
      begin
        Player.SendClientMessage('Sua aliança ja esta cadastrada como defesa.');
        Exit(False);
      end;
    end;
  end;
  ...
  begin
    Player.SendClientMessage('Não existe mais espaço para novas alian�as atacantes.');
    Exit(False);
  end;
  if(Servers[Player.ChannelIndex].CastleSiegeHandler.WarInProgress) then
  begin
    Player.SendClientMessage('Voce nao pode se registrar com a guerra em andamento.');
    Exit(False);
  end;
  if(xGuild.Ally.Guilds[0].Index <> 0) then
  begin
    System.AnsiStrings.StrPLCopy(Nations[xGuild.Nation-1].Cerco.Atacantes[EmptyAttackSlot].LordMarechal, xGuild.Name, 18);
  end;
  if(xGuild.Ally.Guilds[1].Index <> 0) then
  begin
    System.AnsiStrings.StrPLCopy(Nations[xGuild.Nation-1].Cerco.Atacantes[EmptyAttackSlot].Estrategista,
      Guilds[Servers[Player.ChannelIndex].GetGuildSlotByID(xGuild.Ally.Guilds[1].Index)].Name, 18);
  end;
  if(xGuild.Ally.Guilds[2].Index <> 0) then
  begin
    System.AnsiStrings.StrPLCopy(Nations[xGuild.Nation-1].Cerco.Atacantes[EmptyAttackSlot].Juiz,
      Guilds[Servers[Player.ChannelIndex].GetGuildSlotByID(xGuild.Ally.Guilds[2].Index)].Name, 18);
  end;
  if(xGuild.Ally.Guilds[3].Index <> 0) then
  begin
    System.AnsiStrings.StrPLCopy(Nations[xGuild.Nation-1].Cerco.Atacantes[EmptyAttackSlot].Tesoureiro,
...
// Verifica se o dia e horário estão dentro do permitido ou se o jogador possui o item 14200
if not (
  ((DayOfWeek(Now) = 2) and (HourOf(Now) >= 21) and (HourOf(Now) < 22)) or // Segunda-feira das 20:00 às 22:00
  ((DayOfWeek(Now) = 4) and (HourOf(Now) >= 21) and (HourOf(Now) < 22)) or // Quarta-feira das 20:00 às 22:00
  ((DayOfWeek(Now) = 6) and (HourOf(Now) >= 21) and (HourOf(Now) < 22))    // Sexta-feira das 20:00 às 22:00
) and not HasItem14200 then
begin
  Player.SendClientMessage('Você só pode acessar o castelo nas segundas, quartas e sextas das 21:00 às 22:00, ou se possuir o item especial.');
  Exit(False);
end;
...
//Exit(True);
  ...
  SpawnPoint.Create(3434, 2598);

  if(Player.Character.GuildSlot = 0) then
  begin
    Player.SendClientMessage('Voc� tem que estar em uma legião para entrar no castelo.');
    Exit;
  end;

  if(Player.Character.Base.Nation = 0) then
  begin
    Player.SendClientMessage('Voc� tem que estar em uma nação para entrar no castelo.');
    Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          Player.SendClientMessage('A guerra so pode se iniciar as 20h todos os s�bados.');
          Exit;
        end;
