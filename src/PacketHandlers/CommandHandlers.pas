...
var
  ...
  Arquivo: TextFile;
begin
  NomeDoLog := GetCurrentDir + '\Logs\Log.txt';
...
begin
  ...
  begin
    try
      ...
      // TargetName := Player.Account.Header.userName;
      // LogItem(Format('foi adicionado um item id: %d quantidade: %d Pelo Player %s Para o Player %s',
     //  [Index, Quanty, TargetName,OtherPlayer]));
      end;
    ...
    Exit;
      ...
      if Index = 5251 then
      begin
        LogItemUsage5251(Format('Foi adicionado um item id: %d quantidade: %d Player %s Na data %s',
          [Index, Quanty, TargetName, DateTimeToStr(Now)]));
      end;

      // Log geral
      LogItem(Format('Foi adicionado um item id: %d quantidade: %d Player %s Na data %s',
        [Index, Quanty, TargetName, DateTimeToStr(Now)]));
    except
      on E: Exception do
      begin
        Player.SendClientMessage('Erro: ' + E.Message);
        Exit;
      end;
    end;
    end;
...
class procedure TCommandHandlers.Disconnect(var Player: TPlayer; Parameter: string);
var
  OtherPlayer: PPlayer;
begin
  Player.SendClientMessage('Comando temporariamente indisponível.');
  if not(Servers[Player.ChannelIndex].GetPlayerByName(Parameter, OtherPlayer)) then
  begin
    Player.SendClientMessage('Jogador não encontrado.');
   // Exit;
  end;
  if (OtherPlayer.Account.Header.AccountType > Player.Account.Header.AccountType) then
  begin
