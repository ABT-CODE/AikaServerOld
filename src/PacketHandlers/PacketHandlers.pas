...
uses
  Player, Packets, SysUtils, AnsiStrings, Clipbrd, MMSystem,
   Generics.Collections, Winsock2,Math,RoyaleHandler,
  ExtCtrls,  Classes;
type
  ...
  public
    ...
    class function ChangeMasterGuild(var Player: TPlayer;
      Buffer: ARRAY OF BYTE): Boolean;
    class function  AddGoldToMarshalGuildChest(Amount: UInt64): Boolean;
...
uses
  ...
  AuctionFunctions,SQL,TeleportHandler;


class function TPacketHandlers. AddGoldToMarshalGuildChest(Amount: UInt64): Boolean;
var
  NationIndex: Integer;
  MarshalGuildIndex: Integer;
  MarshalGuild: PGuild;
  i: Integer;
begin
  // Percorre todas as nações
  for NationIndex := 0 to High(Nations) do
  begin
    // Pega o índice da guilda Marechal dessa nação
    MarshalGuildIndex := Nations[NationIndex].MarechalGuildID;

    // Verifica se existe uma guilda cadastrada como Marechal
    if (MarshalGuildIndex > 0) and (MarshalGuildIndex <= High(Guilds)) then
    begin
      // Obtém a referência da guilda
      MarshalGuild := @Guilds[MarshalGuildIndex];

      // Adiciona o ouro diretamente ao baú
      MarshalGuild.Chest.Gold := MarshalGuild.Chest.Gold + Amount;

      // Salva as alterações da guilda
      TFunctions.SaveGuilds(MarshalGuildIndex);

      // Notifica todos os membros online sobre a atualização do baú
      {for i := 0 to MaxGuildMembers - 1 do
      begin
        if (MarshalGuild.Members[i].Connected) and (MarshalGuild.Members[i].CharIndex > 0) then
        begin
          TPlayer(MarshalGuild.Members[i].CharIndex).RefreshGuildChestGold;
        end;
      end;}
    end;
  end;
...
procedure AddGoldToArchonGuildChest(Amount: UInt64);
var
  NationIndex: Integer;
  TacticianGuildIDIndex: Integer;
  TacticianGuildID: PGuild;
  i: Integer;
begin
  // Percorre todas as nações
  for NationIndex := 0 to High(Nations) do
  begin
    // Pega o índice da guilda do Archon dessa nação
    TacticianGuildIDIndex := Nations[NationIndex].TacticianGuildID;

    // Verifica se existe uma guilda cadastrada como Archon
    if (TacticianGuildIDIndex > 0) and (TacticianGuildIDIndex <= High(Guilds)) then
    begin
      // Obtém a referência da guilda
      TacticianGuildID := @Guilds[TacticianGuildIDIndex];

      // Adiciona o ouro diretamente ao baú
      TacticianGuildID.Chest.Gold := TacticianGuildID.Chest.Gold + Amount;

      // Salva as alterações da guilda
      TFunctions.SaveGuilds(TacticianGuildIDIndex);

      // Notifica todos os membros online sobre a atualização do baú
     { for i := 0 to MaxGuildMembers - 1 do
      begin
        if (ArchonGuild.Members[i].Connected) and (ArchonGuild.Members[i].CharIndex > 0) then
        begin
          TPlayer(ArchonGuild.Members[i].CharIndex).RefreshGuildChestGold;
        end;
      end; }
    end;
  end;
...
procedure AddGoldToJudgeGuildIDChest(Amount: UInt64);
var
  NationIndex: Integer;
  JudgeGuildIDIndex: Integer;
  JudgeGuildID: PGuild;
  i: Integer;
begin
  // Percorre todas as nações
  ...
  begin
    // Pega o índice da guilda do Archon dessa nação
    JudgeGuildIDIndex := Nations[NationIndex].JudgeGuildID;

    // Verifica se existe uma guilda cadastrada como Archon
    if (JudgeGuildIDIndex > 0) and (JudgeGuildIDIndex <= High(Guilds)) then
    begin
      // Obtém a referência da guilda
      JudgeGuildID := @Guilds[JudgeGuildIDIndex];

      // Adiciona o ouro diretamente ao baú
      JudgeGuildID.Chest.Gold := JudgeGuildID.Chest.Gold + Amount;

      // Salva as alterações da guilda
      TFunctions.SaveGuilds(JudgeGuildIDIndex);
...
procedure AddGoldTotreasurerGuildIDDChest(Amount: UInt64);
var
  NationIndex: Integer;
  treasurerGuildIDIndex: Integer;
  treasurerGuildID: PGuild;
  i: Integer;
begin
  // Percorre todas as nações
  for NationIndex := 0 to High(Nations) do
  begin
    // Pega o índice da guilda do Archon dessa nação
    treasurerGuildIDIndex := Nations[NationIndex].treasurerGuildID;

    // Verifica se existe uma guilda cadastrada como Archon
    if (treasurerGuildIDIndex > 0) and (treasurerGuildIDIndex <= High(Guilds)) then
    begin
      // Obtém a referência da guilda
      treasurerGuildID := @Guilds[treasurerGuildIDIndex];

      // Adiciona o ouro diretamente ao baú
      treasurerGuildID.Chest.Gold := treasurerGuildID.Chest.Gold + Amount;

      // Salva as alterações da guilda
      TFunctions.SaveGuilds(treasurerGuildIDIndex);
...
procedure LogTeleportFraude(CharacterName: String);
...
begin
  ...
  LogFile := GetCurrentDir + '\Logs\teleport\TeleportFraudeLog.txt';

  // Cria a pasta se não existir
  if not DirectoryExists(GetCurrentDir + '\Logs\teleport') then
    ForceDirectories(GetCurrentDir + '\Logs\teleport');

  // Abre ou cria o arquivo
  AssignFile(Arquivo, LogFile);
  ...
  try
    LogEntry := Format('[%s] - Player: %s (Conta: %s) tentou usar o teleporte fora da área permitida. Posição atual: X=%d Y=%d',
      [FormatDateTime('yyyy-mm-dd hh:nn:ss', Now), CharacterName]);
    WriteLn(Arquivo, LogEntry);
    WriteLn(Arquivo, '-------------------------------------------------------------------------------');
  finally
    CloseFile(Arquivo);
...
var
  ...
  Cid: Word;
  dcs: Boolean;
  PlayerPtr: PPlayer; // Ponteiro para o jogador
...
begin
  Result := False;
  PlayerPtr := @Player; // Captura o endereço do Player

  // Verificação da versão do cliente
  if (Packet.Version <> SERVER_VERSION) then
  begin
    Player.SendClientMessage('Client desatualizado. Abrir pelo Launcher como Administrador.');
    Logger.Write(Packet.Version.ToString, TLogType.Packets);
    DisconnectPlayer;
    Exit;
  end;

  // Carregar informações da conta do jogador
  ...
  begin
    Player.SendClientMessage('Conta bloqueada. Entre em contato com o suporte.');
    DisconnectPlayer;
    Exit;
  end;


  // Verificar se o jogador está conectado em outro servidor
  dcs := False;
  ...
  // Se houve desconexão em outro servidor, desconecta o jogador atual
  if dcs then
  begin
  ...
  // Autenticar jogador e enviar lista de personagens
  Player.Account.Header.IsActive := True;
  Logger.Write('[' + string(ServerList[Player.ChannelIndex].Name) + ']: Usuário conectado [' + Username + '].', TLogType.ConnectionsTraffic);
...
var
  ...
  function DateTimeToUnix(const ADateTime: TDateTime): Int64;
  begin
    // Converte TDateTime para Unix Timestamp
...
begin
  ...
  begin
    ...
    begin
      Player.SendCharList;
      Player.SendClientMessage('Numérica não correspondente!', 16);
      Exit;
    end;

    // Cancela exclusão se já estava marcada
    ...
    begin
      ...
      // Valida a numérica do personagem
      ...
      // Marca exclusão do personagem e define a data em formato Unix Timestamp
      if (Player.Account.CharactersDelete[RecvPacket.SlotIndex] = False) then
      begin
        FTime := IncDay(Now, DELETE_DAYS_INC); // Incrementa dias para exclusão
        UnixTimestamp := DateTimeToUnix(FTime); // Converte para Unix Timestamp

        // Atualiza os dados na memória
        Player.Account.CharactersDelete[RecvPacket.SlotIndex] := true;
        Player.Account.CharactersDeleteTime[RecvPacket.SlotIndex] := IntToStr(UnixTimestamp);
        ...
        // Atualiza visualmente a lista de personagens no cliente
        Player.SendCharList;
        ...
        Player.SendClientMessage('Personagem marcado para exclusão com sucesso!', 16);
      ...
      // Valida se a data de exclusão já foi atingida
      try
        UnixTimestamp := StrToInt64(Player.Account.CharactersDeleteTime[RecvPacket.SlotIndex]);
        FTime := UnixTimestamp / 86400 + EncodeDate(1970, 1, 1);
      except
        Player.SendClientMessage('Erro ao processar a data de exclusão.', 16);
        Exit;
      end;
...
begin
  ...
  //  Exit;
  // Verifica se o jogador está em party e não se moveu por 2 minutos

  // Verificação de inatividade em party (2 minutos sem movimento)
  ...
  begin
    if(Player.Base.PlayerCharacter.LastPos.Distance(Packet.Destination) >
      (Round(SecIntoMoving / 1000) * (Player.Base.PlayerCharacter.SpeedMove / 1.5))) then
    begin
  ...
  if(Player.Base.PlayerCharacter.LastPos.Distance(Packet.Destination) >
      (Player.Base.PlayerCharacter.SpeedMove / 3.5)) then
  begin
  ...
  if(Player.Base.IsDead) then
    Exit;
  if((Packet.Destination.X <= 10) or (Packet.Destination.Y <= 10)
    or not(Packet.Destination.IsValid)) then
    Exit;
...
begin
  ...
  if (Player.Character.Rotation = Packet.Data) then
    Exit;
  if((Packet.Data >= 0) and (Packet.Data <= 360)) then
  begin
    Player.Character.Rotation := Packet.Data;
  end
  else
  begin
    Player.Character.Rotation := 0;
    Packet.Data := Player.Character.Rotation;
  end;
...
var
  ...
  // Verificação específica para o NPC 2235
    if (Packet.Index = 2235) then
    begin
      // Verifica se o jogador possui o buff 6600 ou 6601
      if not Player.base.BuffExistsByID(6600) then
      begin
        Player.SendClientMessage('Somente o Marechal pode acessar essa Loja.');
        Exit;
      end;
    end;
  ...
  begin
    ...
    3373: //bandeira do marechal 3584 2805
      begin
        if (Servers[Player.ChannelIndex].CastleSiegeHandler.OrbsHolded < 0) then
        begin
      ...
      3374: //bandeira do marechal 3584 2805
      begin
        if (Servers[Player.ChannelIndex].CastleSiegeHandler.OrbsHolded < 0) then
        begin
          Player.SendClientMessage('As orbes precisam ser ocupadas antes de iniciar o selo');
  ...
  case Packet.Type1 of
    ...
    $23:
      begin
        ...
        TNPCHandlers.marechalbuff(Player, Packet.Index);
    ...
    begin
        Player.OpennedNPC := 0;
        Player.OpennedOption := 0;
        //Player.SendClientMessage('A benção por honra está indisponível.');
        //Exit;

        if Player.Base.Character.CurrentScore.Honor < 50000 then
        begin
            Player.SendClientMessage('Você não possui honra suficiente.');
            Exit;
        end;

        Player.Base.Character.CurrentScore.Honor :=
        Player.Base.Character.CurrentScore.Honor - 50000;
       Player.Base.SendRefreshKills();


         Player.Base.AddBuff(272, True, false, 3000); // Exemplo: buff leve
         Player.Base.AddBuff(208, True, False, 3000);
         Player.Base.AddBuff(352, True, false, 3000); // Exemplo: buff leve
         Player.Base.AddBuff(4960, True, False, 3000);
         Player.Base.AddBuff(5002, True, false, 3000); // Exemplo: buff leve
         Player.Base.AddBuff(5162, True, False, 3000);
         Player.Base.AddBuff(5194, True, False, 3000);
         Player.Base.AddBuff(1463, True, False, 3000);

        Player.SendClientMessage('Você foi abençoado copm Buffs que custaram 50k de honrra .');
    ...
    67: //ursula
      begin //3087 3621
        Player.Teleport(TPosition.Create(393, 2180));
        Player.OpennedNPC := 0;
        Player.OpennedOption := 0;
        Exit;
      end;
    ...
    68: //basilan
      begin //1635 2218
        ...
        begin
          Player.DecGold(2500);
          Player.Teleport(TPosition.Create(1893, 3787));
        end;
        Player.OpennedNPC := 0;
        Player.OpennedOption := 0;
        Exit;
    ...
    79: //Embate dos Destemidos  Zona 1
    begin
           if (Player.Base.Character.Nation > 0) then
           begin
            ...
            begin
             Player.SendClientMessage
             ('Só pode ser utilizado em Leopold.');
            Exit;
              end;


           {if (HourOf(Now) < 1) or (HourOf(Now) >= 24) then
              begin
                Player.SendClientMessage('Você só entrar no Embate dos Destemidos a partir das 01:00 até 00:00.');
                Exit(False);
              end;}


              begin //1635, 1415
                  Player.Teleport(TPosition.Create(1635, 1415));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
      ...
      83: //Embate dos Destemidos  Zona 2
    begin
           if (Player.Base.Character.Nation > 0) then
           begin
             if (  Servers[Player.ChannelIndex]
           ...
           {if (HourOf(Now) < 01) or (HourOf(Now) >= 24) then
              begin
                Player.SendClientMessage('Você só entrar no Embate dos Destemidos a partir das 01:00 até 00:00.');
                Exit(False);
              end;  }
              ...
              begin //1762 1348
                  Player.Teleport(TPosition.Create(1762, 1348));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
       ...
       84: //Embate dos Destemidos Zona 3
    begin
           if (Player.Base.Character.Nation > 0) then
           begin
             if (  Servers[Player.ChannelIndex]
           ...
           {if (HourOf(Now) < 1) or (HourOf(Now) >= 24) then
              ...
              begin //1635, 1415
                  Player.Teleport(TPosition.Create(1591, 1509));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
   ...
   80: // Coliseu
    begin
      if (Player.Base.Character.Nation > 0) then
        begin
          if (Servers[Player.ChannelIndex].NationID <> 4) then
            begin
              Player.SendClientMessage('Só pode ser utilizado em Leopold.');
              Exit;
            end;

          begin
             var
             LastAccessTime: TDateTime; // Variável global ou do escopo necessário

             const
             INTERVALO_MINUTOS = 10; // Intervalo de 5 minutos para acessar a loja

              begin
                // Inicializa LastAccessTime se necessário
                if LastAccessTime = 0 then
                  LastAccessTime := Now;

                // Obtém a hora atual
                var CurrentHour := HourOf(Now);
                var CurrentMinute := MinuteOf(Now);
                var CurrentSecond := SecondOf(Now);
                // Verifica se a hora atual está dentro do intervalo permitido (6:00 a 23:00)
                if (CurrentHour < 1) or (CurrentHour > 24) then
                begin
                  Player.SendClientMessage('O Batlle Royall esta fechado . Horário de funcionamento: 1:00 - 24:00 com um intervalo de 10 minutos a cada hora.');
                  Exit(False);
                end;
                // Verifica se o acesso está dentro dos 5 minutos após o início da hora
                if (CurrentMinute < 0) or (CurrentMinute >= 10) then
                begin
                  Player.SendClientMessage('Você só pode acessar O Batlle Royalle 1:00 e 24:00 com um intervalo de 10 minutos a cada hora.');
                  Exit(False);
                end;
              end;
          end;
          // Lista de possíveis localizações para teleportação
          var PossibleLocations: array[0..11] of TPosition;
          PossibleLocations[0] := TPosition.Create(3143, 3039);
          PossibleLocations[1] := TPosition.Create(3161, 3052);
          PossibleLocations[2] := TPosition.Create(3179, 3042);
          PossibleLocations[3] := TPosition.Create(3200, 3046);
          PossibleLocations[4] := TPosition.Create(3197, 3028);
          PossibleLocations[5] := TPosition.Create(3215, 3028);
          PossibleLocations[6] := TPosition.Create(3218, 3017);
          PossibleLocations[7] := TPosition.Create(3219, 2985);
          PossibleLocations[8] := TPosition.Create(3310, 2906);
          PossibleLocations[9] := TPosition.Create(3282, 2975);
          PossibleLocations[10] := TPosition.Create(3315, 3019);

          // Escolhe uma localização aleatória da lista
          Randomize; // Inicializa o gerador de números aleatórios
          var RandomIndex := Random(Length(PossibleLocations)); // Gera um índice aleatório
          var ChosenLocation := PossibleLocations[RandomIndex]; // Seleciona a localização com base no índice

          // Remove todos os buffs e atualiza os pontos do jogador
         // RemoveAllBuffsAndUpdate(Player.Base);

          // Registra o jogador no sistema Battle Royale (independente da verificação)
          // TRoyaleHandler.RegisterPlayer(Player.Base);

          // Chama o método ProcessTeleport passando Player como TBaseMob
          TTeleportHandler.ProcessTeleport(Player.Base);

          // Altera a nação do jogador para Nação 4 (opcional, se necessário)
          // Player.Base.Character.Nation := 4;
          // Player.Base.Character.GuildIndex := 0;

          // Teleporta o jogador para a localização escolhida
          Player.Teleport(ChosenLocation);


           // Remove os buffs ao jogador
            Player.Base.RemoveBuff(6498);
            Player.Base.RemoveBuff(6499);
            Player.Base.RemoveBuff(207);
            Player.Base.RemoveBuff(271);
            Player.Base.RemoveBuff(348);
            Player.Base.RemoveBuff(5163);
            Player.Base.RemoveBuff(5193);
            Player.Base.RemoveBuff(1461);
            // buff de honrra
            Player.Base.RemoveBuff(6464);
            Player.Base.RemoveBuff(6643);
            Player.Base.RemoveBuff(6701);
            Player.Base.RemoveBuff(7068);
            Player.Base.RemoveBuff(7236);
            Player.Base.RemoveBuff(7249);
            Player.Base.RemoveBuff(7893);
            Player.Base.RemoveBuff(7896);
            Player.Base.RemoveBuff(7916);




          // Aplica os novos buffs ao jogador
          Player.Base.AddBuff(9177, True, False, 50); // Buff específico 1
          Player.Base.AddBuff(7365, True, False, 50); // Buff específico 2
          Player.Base.AddBuff(4047, True, False, 50); // Buff específico 3
          Player.Base.AddBuff(2096); // Buff específico 3
           Player.Base.AddBuff(9185); // Buff Batlle royalle



          // Limpa o NPC e as opções abertas pelo jogador
          Player.OpennedNPC := 0;
          Player.OpennedOption := 0;

          // Mensagem de sucesso para o jogador
          {Servers[Player.ChannelIndex].SendServerMsgForNation(
            AnsiString('Você foi registrado no evento Battle Royale e teleportado para a arena!'),
            Player.Base.Character.Nation,
            16, 16, 16
          );}
          ...
          (' entrou no evento Battle Royale e foi teleportado para a arena!' ),16,16,16) ;
      ...
      82: //caverna Hecla inferior
    begin // 1382 1869
           if (Player.Base.Character.Nation > 0) then

              begin //1382, 1869
                  Player.Teleport(TPosition.Create(3779, 3679));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
      ...
      81: //teleporte Caverna Hecla parte 2
    begin // 3615 3635
           if (Player.Base.Character.Nation > 0) then

              begin //3615 3635
                  Player.Teleport(TPosition.Create(3615, 3635));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin // 2144 3647
           ...
           if (  Servers[Player.ChannelIndex]
             ...
             const
             INTERVALO_MINUTOS = 5; // Intervalo de 5 minutos para acessar a loja
              ...
              begin
                ...
                var CurrentSecond := SecondOf(Now);
                // Verifica se a hora atual está dentro do intervalo permitido (6:00 a 23:00)
                if (CurrentHour < 6) or (CurrentHour > 23) then
                begin
                  Player.SendClientMessage('A loja está fechada. Horário de funcionamento: 6:00 - 23:00.');
                  Exit(False);
                end;
                // Verifica se o acesso está dentro dos 5 minutos após o início da hora
                if (CurrentMinute < 0) or (CurrentMinute >= 5) then
                begin
                  Player.SendClientMessage('Você só pode acessar a loja entre 6:00 e 23:00 com um intervalo de 5 minutos a cada hora.');
                  Exit(False);
                end;
                // Verifica se o tempo desde o último acesso é menor que uma hora
                {if (Now - LastAccessTime) < (1/24) then // Checa se passaram menos de 1 hora (1/24 de um dia)
                begin
                  Player.SendClientMessage('Você só pode entrar na loja a cada 1 hora.');
                  Exit(False);
                end;}
              ...
              begin //2431, 3620
                  Player.Teleport(TPosition.Create(2144, 3647));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin // 2076 3648
           ...
           if (  Servers[Player.ChannelIndex]
             ...
             INTERVALO_MINUTOS = 5; // Intervalo de 5 minutos para acessar a loja
              ...
              begin //2434,3800
                  Player.Teleport(TPosition.Create(2076, 3648));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin
           ...
           {if (HourOf(Now) < 1) or (HourOf(Now) >= 24) then
              begin
                Player.SendClientMessage('Você só entrar na DG a partir das 01:00 até 00:00.');
                Exit(False);
              end;  }


              begin //1635, 1415
                  Player.Teleport(TPosition.Create(3086, 3193));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin // 2947 1664
           ...
           begin
              ...
              begin //2947 1664
                  Player.Teleport(TPosition.Create(2887, 1564));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin
           ...
           begin
            ...
            Exit;
              ...
              begin //1635, 1415
                  Player.Teleport(TPosition.Create(2682, 679));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin
           ...
           {if (HourOf(Now) < 1) or (HourOf(Now) >= 24) then
              ...
              begin //1635, 1415
                  Player.Teleport(TPosition.Create(2320, 2691));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
     ...
     53:   //Teleporte Valer do Silencio
    begin
           if (Player.Base.Character.Nation > 0) then
           begin
             if (  Servers[Player.ChannelIndex]
            ...
            Exit;
              ...
              begin //1635, 1415
                  Player.Teleport(TPosition.Create(875,614));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin
           ...
           {if (HourOf(Now) < 1) or (HourOf(Now) >= 24) then
              ...
              begin //1635, 1415
                  Player.Teleport(TPosition.Create(2883, 2902));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin
           ...
           {if (HourOf(Now) < 1) or (HourOf(Now) >= 24) then
              ...
              begin //1635, 1415
                  Player.Teleport(TPosition.Create(665 , 2659));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin
       ...
       14: //teleporte DG MINA
    begin // 3615 3635
           if (Player.Base.Character.Nation > 0) then

              begin //3615 3635
                  Player.Teleport(TPosition.Create(2901,3547));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
    ...
    begin
           ...
           {if (HourOf(Now) < 1) or (HourOf(Now) >= 24) then
              ...
              begin //1635, 1415
                  Player.Teleport(TPosition.Create(3438,3264));
                  Player.OpennedNPC := 0;
                  Player.OpennedOption := 0;
                  Exit;
              end;
      ...
      for i := 0 to 59 do
      begin
        if (Player.Base.Character.Inventory[i].Index = 18586) then
        begin
          ItemFound := True;
          Break;
        end;
      ...
      begin
        Player.SendClientMessage('Você precisa do Pergaminho do Marechal para Obter o Buff .');
        Exit;
      end;

        Player.Base.AddBuff(7364);
...
begin
  ...
  // Verificação para garantir que apenas o personagem no slot 0 com o item 8250 no inventário possa acessar o NPC 2296
  if (Packet.Index = 2296) then
  begin
    if Player.SelectedCharacterIndex < 0 then
    begin
      Player.SendClientMessage('Apenas o personagem no slot 1 pode interagir com este NPC.');
      Exit;
    end;



    // Verificar se o item 8250 está no inventário do personagem
    ItemFound := False;
    for i := 0 to 59 do
    begin
      if (Player.Base.Character.Inventory[i].Index = 8250) then
      begin
        ItemFound := True;
        Break;
      end;
    ...
    if not ItemFound then
    begin
      Player.SendClientMessage('Você precisa Auxílio no inventário para Comprar com este NPC.');
      Exit;
    end;
  end;
  ...
  begin
    ...
    begin
      Player.SendClientMessage('Não é usuário do Aika Tem. Compre o auxílio na loja premium Wars.');
      Exit;
    end;
...
begin
  ...
  begin
    ...
    begin
      SellPrice := 1;
      Player.SendClientMessage('Este item tem valor zero. Você receberá apenas 100 ouro.');
    end;
    ...
    AddGoldToMarshalGuildChest(RandomRange(TAXA_MAXIMAM , TAXA_MINIMA ));
    AddGoldTotreasurerGuildIDDChest(RandomRange(TAXA_MAXIMAA , TAXA_MINIMAA ));
    AddGoldToJudgeGuildIDChest(RandomRange(TAXA_MAXIMAA , TAXA_MINIMAA ));
    AddGoldToArchonGuildChest(RandomRange(TAXA_MAXIMAA , TAXA_MINIMAA ));
  ...
  begin
    ...
    begin
     ...
     if ItemList[Item.Index].SellPrince <= 0 then
      ...
      if(ItemList[Item.Index].TypeTrade in [0, 1, 2]) then
      begin
...
{$REGION 'Change APP'}
    CHANGE_APP:
      begin
        ...
        if (Player.Character.Base.Gold < 50000) then
        begin
        ...
        if (Reinforced = 2) then
        begin
          Dec(Player.Character.Base.Gold, 50000);
          Result := true;
        end;
      end;
{$ENDREGION}
...
begin
  ...
  begin
    Logger.Write('Falha de conexão individual com MySQL.[ReclaimCoupom]', TLogType.Warnings);
    Logger.Write('PERSONAL MYSQL FAILED LOAD.[ReclaimCoupom]', TLogType.Error);
    SQLComp.Destroy;
    Exit;
  end;
  ...
  // Consulta o cupom no banco de dados
  SQLComp.SetQuery('SELECT id, target_itemid, command_type, target_itemcnt, site, id_site, created_at, runned FROM gm_commands ' +
                   'WHERE coupom = ' + QuotedStr(RealCupom) +
                   ' AND refused = 0 AND target_name = ""');
  SQLComp.Run();

  if SQLComp.Query.RecordCount = 0 then
  begin
    Player.SendClientMessage('Cupom já utilizado ou inexistente.');
    SQLComp.Destroy;
    Exit;
  end;
  ...
  CoupomSite := SQLComp.Query.Fields[4].AsInteger;
  CoupomOwnerID := SQLComp.Query.Fields[5].AsInteger;
  CreatedAt := SQLComp.Query.Fields[6].AsDateTime;
  IsRunned := SQLComp.Query.Fields[7].AsInteger;

  // Verifica se o cupom já foi resgatado
  if IsRunned = 1 then
  begin
    Player.SendClientMessage('Este cupom já foi resgatado.');
    SQLComp.Destroy;
    Exit;
  end;

  // Verifica se já passaram 7 dias desde a criação (expiração)
  if Now > CreatedAt + 7 then
  begin
    Player.SendClientMessage('Este cupom expirou e não pode mais ser utilizado.');

    // Atualiza o banco de dados para marcar como expirado
    SQLComp.SetQuery('UPDATE gm_commands SET runned = 1, runned_at = NOW() WHERE id = ' + CommandID.ToString);
  ...
  // Se o cupom for do tipo `site = 1`, verificar o ID da conta do jogador
  if CoupomSite = 1 then
  begin
    if Player.Account.Header.AccountID <> CoupomOwnerID then
    begin
      Player.SendClientMessage('Este cupom só pode ser usado pela conta que o gerou.');
      SQLComp.Destroy;
      Exit;
    end;
  end;

  // Processa o cupom baseado no tipo de comando
  case SQLComp.Query.Fields[2].AsInteger of
    1:  // Adiciona Cash ao jogador
      begin
        CashAmount := SQLComp.Query.Fields[1].AsInteger * 1000;
        Player.AddCash(CashAmount);
        Player.SendPlayerCash;
        Player.SendClientMessage('Foi entregue em sua conta a quantidade de [' + CashAmount.ToString + ' Cash!].');
      end;

    2:  // Adiciona Item ao jogador
      begin
  ...
  // Atualiza o banco de dados marcando o cupom como resgatado
  SQLComp.SetQuery('UPDATE gm_commands SET target_name = ' +
                   QuotedStr(String(Player.Base.Character.Name)) +
                   ', runned = 1, runned_at = NOW() WHERE id = ' + CommandID.ToString);
  SQLComp.Query.Connection.StartTransaction;
  SQLComp.Run(False);
  SQLComp.Query.Connection.Commit;
...
{$REGION 'Attack & Skill Functions'}
   class function TPacketHandlers.UseSkill(var Player: TPlayer;
    ...
    begin
      ...
      if not ((SkillData[Packet.Skill].Classe >= 61) and (SkillData[Packet.Skill].Classe <= 84)) then
      begin
        if Player.Base.BuffExistsByIndex(77) then
          Player.Base.RemoveBuffByIndex(77);
        if Player.Base.BuffExistsByIndex(53) then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Player.Base.Target := @Servers[Player.ChannelIndex].Players[Packet.Index].Base;
          end;
        end
        ...
        begin
          ...
          begin
            if OtherMob.IsDead then Exit;
            case Packet.Index of
              3340..3354: Packet2.TargetPos := Servers[Player.ChannelIndex].DevirStones[OtherMob.ClientID].PlayerChar.LastPos;
              3355..3369: Packet2.TargetPos := Servers[Player.ChannelIndex].DevirGuards[OtherMob.ClientID].PlayerChar.LastPos;
            else
          ...
          begin
            ...
            OtherMob := @Servers[Player.ChannelIndex].MOBS.TMobS[mobid].MobsP[mobpid].Base;
        ...
        FillChar(Bufferx[0], SizeOf(Bufferx), 0);
      ...
      begin
        mobid := TMobFuncs.GetMobGeralID(Player.ChannelIndex, Packet.Index, mobpid);
        if mobid <> -1 then // Verifica se a ID do mob é válida
        begin
          OtherMob := @Servers[Player.ChannelIndex].MOBS.TMobS[mobid].MobsP[mobpid].Base;
          Player.Base.Target := OtherMob;
          Player.Base.CheckCooldown(Packet);
          Result := True;
          Exit;
        end
        ...
        begin
          // Adicione um log ou tratamento de erro se necessário
          Logger.Write('ID do mob não encontrado.', TLogType.error);
        end;
      end;
      ...
      if (Player.Base.ClientId = Player.Base.Target.ClientId) and (SkillData[Packet.Skill].Index <> 113) then
      begin
...
var
  Taxa: Integer;
  I: BYTE;
  PontosAdicionaisSkill: integer;
  PontosAdicionaisStatus: integer;
begin
  Result := False;
  ...
  Player.Character.Base.CurrentScore.SkillPoint :=
    ...
    // Adiciona pontos extras para níveis acima de 85
  {if Player.Base.Character.Level >= 85 then
  begin
    PontosAdicionaisSkill := (Player.Base.Character.Level - 84) * SKILL_POINT_EXTRA;
    PontosAdicionaisStatus := (Player.Base.Character.Level - 84) * STATUS_POINT_EXTRA;

    Inc(Player.Character.Base.CurrentScore.SkillPoint, PontosAdicionaisSkill);
    Inc(Player.Character.Base.CurrentScore.Status, PontosAdicionaisStatus);
  end; }
...
begin
  ...
  // não permitir o pk em mebros da mesma guild
  ...
  begin
    ...
    case Player.Base.AttackMsgCount of
      1:
        begin
          Player.SendClientMessage('Atenção!Velocidade de Dano Acima do Limite Risco de Banimento.');

        end;
      2:
        begin
          Player.SendClientMessage('Atenção 2!Velocidade de Dano Acima do Limite Risco de Banimento.');
      ...
      3:
        begin
          Player.SendClientMessage(' Velocidade de Dano Acima do Limite.');
          Player.Base.AttackMsgCount := 3;
          Player.Base.LastAttackMsg := Now;

          // Registra o log da infração
          LogInfo(player.base.Character.Name, 'Velocidade de Dano Acima do Limite. ');
          Player.SocketClosed := True;
  ...
  if (MilliSecondsBetween(Now, Player.Base.LastAttackMsg) <= MIN_DELAY_ATTACK + 500 ) then
  begin
    Servers[Player.ChannelIndex].Disconnect(Player);
    Player.SocketClosed := True;
    Exit;
  ...
  //Começo do que tava Comentrado

{if not(Player.Base.CheckCooldown2(Packet.Skill)) then
begin
  // Incrementa o contador de uso de habilidades enquanto estão em cooldown
  inc(Player.Base.LongSkillTimes);

  // Verifica se o jogador excedeu o limite permitido
  if (Player.Base.LongSkillTimes >= 5) then
  begin
    // Desconecta o jogador
    Player.Disconnect;
    Exit;
  ...
  // Envia os tempos de cooldown para o cliente
  Player.Base.SendCurrentAllSkillCooldown();
  Exit;
end
else
begin
  // Reseta o contador de uso quando a habilidade não está em cooldown
  Player.Base.LongSkillTimes := 0;
end;}


  // final do que tava Comentado
  ...
  begin // skills de ataque em area[targets]
   ...
   // código anterior data 29/07/2024
   if not(DataSkill^.CastTime > 0) then
    begin
      if not(Player.Base.CheckCooldown2(Packet.Skill)) then
      begin
        Exit;
...
begin
  ...
  // Verifica se o player está próximo do NPC (ID 2129)
  if not (Player.Base.PlayerCharacter.LastPos.InRange(TPosition.Create(2947, 1664), 3)) then
  begin
    Player.SendClientMessage('Utilização de Teleporte fora dos Padrões. Rico de Banimento.');

       // Gera log detalhado da tentativa
    LogTeleportFraude(
  ...
  //ChangeChannelToken.titleFromOther := Player.Base.PlayerCharacter.Titles;
  if(Player.Base.PetClientID <> 0) then
    Player.Base.DestroyPet(Player.Base.PetClientID);
  ...
  case Packet.Stage of
    0:
      begin
        ...
        // Verifica se o jogador possui o item 14204 no inventário
        ItemFound := False;
        for i := 0 to High(Player.Base.Character.Inventory) do
        begin
          if (Player.Base.Character.Inventory[i].Index = 14204) then
          begin
            ItemFound := True;
            Break;
          end;
        ...
        if not ItemFound then
        begin
          // Segue a lógica normal de criação de guild (com grupo)
          if(Player.PartyIndex = 0) then
          begin
            Player.SendClientMessage('Você não está em um grupo.');
            Exit;
          end;
          if Player.Party.Members.Count < 5 then
          begin
            Player.SendClientMessage('Você precisa de um grupo com 5 pessoas para criar a guild.');
            Exit;
          end;
          ...
          begin
            PartyMember := @Servers[Player.ChannelIndex].Players[PartyMemberIndex];
            if PartyMember.Character.Base.Nation <= 0 then
            begin
              Player.SendClientMessage('Todos os membros do grupo devem ter nação.');
              Exit;
            end;
            if Player.Base.PlayerCharacter.LastPos.InRange
              (PartyMember.Base.PlayerCharacter.LastPos, 25) = False then
            begin
              Player.SendClientMessage('Todos os membros do grupo devem estar perto.');
              Exit;
            end;
            if PartyMember.Character.Base.Level < 95 then
            begin
              Player.SendClientMessage('Todos os Membros precisam estar no nivel 95.');
              Exit;
            end;
          end;
        end
        ...
        begin
          // Mensagem indicando que o jogador pode criar a guild sozinho
          Player.SendClientMessage('Você tem o item especial e pode criar uma guild sozinho.');
        end;

        if Player.Character.Base.Gold < Taxa then
        begin
          Player.SendClientMessage('Você não tem gold suficiente para criar uma guild.');
          Exit;
        end;
        if Player.Character.Base.Level < 95 then
        begin
          Player.SendClientMessage('O level mínimo para criar uma guild é 95.');
          Exit;
        end;
...
var
  ...
  Attributes: array[0..4] of Integer; // Array para armazenar os valores dos atributos
  i, j: Integer;
  Temp: Integer;
...
begin
  ...
  // Verifica se o valor total dos atributos não ultrapassa 4400
  TotalAttributes := Player.Character.Base.CurrentScore.Str +
                     Player.Character.Base.CurrentScore.Agility +
                     Player.Character.Base.CurrentScore.Int +
                     Player.Character.Base.CurrentScore.Cons +
                     Player.Character.Base.CurrentScore.Luck;

  if (TotalAttributes + Packet.StatusAmount) > 4300 then
  begin
    Player.SendClientMessage('Você não pode ultrapassar o limite total .');
    Exit;
  end;
  ...
  // Armazena os valores dos atributos em um array para ordenação
  Attributes[0] := Player.Character.Base.CurrentScore.Str;
  Attributes[1] := Player.Character.Base.CurrentScore.Agility;
  Attributes[2] := Player.Character.Base.CurrentScore.Int;
  Attributes[3] := Player.Character.Base.CurrentScore.Cons;
  Attributes[4] := Player.Character.Base.CurrentScore.Luck;

  // Ordena o array em ordem decrescente
  for i := 0 to High(Attributes) - 1 do
    for j := i + 1 to High(Attributes) do
  ...
  // Verifica as regras de valores máximos
  if (Attributes[0] > 1500) or  // O maior atributo não pode ultrapassar 1500
     (Attributes[1] > 1000) or  // O segundo maior não pode ultrapassar 1000
     (Attributes[2] > 800) or   // O terceiro maior não pode ultrapassar 800
     (Attributes[3] > 600) or   // O quarto maior não pode ultrapassar 600
     (Attributes[4] > 400) then // O quinto maior não pode ultrapassar 400
  begin
    // Se alguma regra for violada, reverte a alteração e retorna False
    case Packet.StatusIndex of
      0: // str
        Dec(Player.Character.Base.CurrentScore.Str, Packet.StatusAmount);
      1: // agility
        Dec(Player.Character.Base.CurrentScore.Agility, Packet.StatusAmount);
      2: // int
        Dec(Player.Character.Base.CurrentScore.Int, Packet.StatusAmount);
      3: // cons
        Dec(Player.Character.Base.CurrentScore.Cons, Packet.StatusAmount);
      4: // luck
        Dec(Player.Character.Base.CurrentScore.Luck, Packet.StatusAmount);
    end;

    // Devolve os pontos para o campo de status
    Inc(Player.Character.Base.CurrentScore.Status, Packet.StatusAmount);

    // Envia uma mensagem ao jogador informando o motivo da falha
   Player.SendClientMessage('Você não pode ultrapassar os limites máximos de atributos.');

    // Desconecta o jogador colocar pontosd amais
    Player.SocketClosed := True;
    Servers[Player.ChannelIndex].Disconnect(Player);
    Exit;
  end;
...
begin
  ...
  // Ajuste para aumentar os limites das taxas em 100%
  if (CitizenTax >= 5) and (CitizenTax <= GOLD_TAXA_CID) then  // Limite aumentado de 15 para 30
  begin
    if (VisitorTax >= 10) and (VisitorTax <= GOLD_TAXA_NCID) then  // Limite aumentado de 20 para 40
    begin
      ...
      6370 .. 6444 , 12000 .. 12032:
        ...
        begin
          ...
          begin
            ...
            begin
              if (Servers[I].Devires[DevirId].Slots
                [SlotId].ItemId <> 0) then
              begin
                if ItemList[Item^.Index].UseEffect = ItemList[Servers[I].Devires[DevirId].Slots
                [SlotId].ItemId].UseEffect
                 then
                begin
                  Player.SendClientMessage('Já possui uma reliquia desse tipo ' +
                    AnsiString(ItemList[Item^.Index].Name) + ' no seu ' + Servers[I].DevirNpc[DevirId+3335].DevirName + ' ou de outra nação.');
                  Exit;
                end;
              end;
            end;
            ...
            //dentro da funcao  moveitemtoreliquare
          ...
          IsRecap := False;
          // depois alterar para false para dar menos honra quando for recap
          for I := Low(Servers) to High(Servers) do
          begin
            if not(Servers[I].IsActive) then
             Continue;
            ...
            begin
              for SlotId  := 0 to 4 do
              begin
                if (Servers[I].Devires[DevirId].Slots[SlotId].Furthed) then
                begin
                  if (Servers[I].Devires[DevirId].Slots[SlotId].ItemFurthed = Item.Index)
                  then
                  begin
            ...
            begin //honra só pra minha party
              ...
              begin
                ...
                begin
                  ...
                  TItemFunctions.PutItem(Servers[Player.ChannelIndex].Players[i], 5987,20);
                  TItemFunctions.PutItem(Servers[Player.ChannelIndex].Players[i], 8480, PENA_DE_GELO_MULT);
                  TItemFunctions.PutItem(Servers[Player.ChannelIndex].Players[i], 10299, 5);
               ...
               if(Player.Party.PartyAllied[j] = 0) then
                ...
                begin
                  ...
                  begin
                    ...
                    TItemFunctions.PutItem(Servers[Player.ChannelIndex].Players[i], 5987,20);
                    TItemFunctions.PutItem(Servers[Player.ChannelIndex].Players[i], 8480, PENA_DE_GELO_MULT);
                      TItemFunctions.PutItem(Servers[Player.ChannelIndex].Players[i], 23399, 5);
                  end;
                end;
              end;
            end;
          ...
          begin
            ...
            TItemFunctions.PutItem(Player, 5987,20);
            TItemFunctions.PutItem(Player, 8480, PENA_DE_GELO_MULT);
            TItemFunctions.PutItem(Player, 10299, 5);
...
begin
  ...
  begin
    ...
    //begin
    Servers[Player.ChannelIndex].SendServerMsgForNation('Jogador ' +
      Player.Base.Character.Name + ' cancelou a orb.',
      Player.Base.Character.Nation);
